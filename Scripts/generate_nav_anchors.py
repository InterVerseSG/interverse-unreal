...
- Runtime-safe tags include both the canonical POI id and navigation_anchor.
...
def _validate_config(config):
    ...
    for entry in config.get("pending_anchors", []):
        ...
        if anchor:
            seen.add(anchor)
    if duplicates:
        raise RuntimeError("Duplicate InterVerse navigation anchors: " + ", ".join(sorted(set(duplicates))))
...
def _create_or_update_anchor(entry):
    ...
    # Actor labels are editor-only. The navigation_anchor tag is what packaged
    # Meta Quest builds use to resolve destinations at runtime.
    actor.tags = [
...
def generate():
    ...
    unreal.log(
        "InterVerseSG NAV summary: created={}, updated={}, skipped={}, pending={}.".format(
            created, updated, skipped, len(pending)
        )
    )
    for entry in pending:
        unreal.log_warning(
            "InterVerseSG pending coordinate: {} ({}) - not placed.".format(
                entry.get("display_name", entry.get("id")), entry.get("navigation_anchor")
            )
        )
